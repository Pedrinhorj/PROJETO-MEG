...
│   ├── knowledge_extractor.py   # Usa engenharia de prompt restritivo para criar um output em JSON nativo no Gemma 4 (4B).
...
- **`knowledge_extractor.py`**: Onde o Gemma 4 (4B) é evocado para analisar texto puro e ser forçado a criar um JSON estruturado. Corta saídas longas proativamente usando truncamento para mitigar limites de token.
...
Lembre-se também de garantir que o modelo `gemma4:e2b` rode no Ollama:
